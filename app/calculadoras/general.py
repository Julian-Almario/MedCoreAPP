...
def calculadora_paquete_año():
    ...
    def calcular_paquete_anio(e):
        try:
            ...
            años = float(años_field.value.replace(',', '.'))

            paquete_año = (cigarrillos * años) / 20

            # Estratificación del riesgo
            if paquete_año < 5:
                riesgo = "Leve"
            elif 5 <= paquete_año < 15:
                riesgo = "Moderado"
            elif 16 <= paquete_año <= 25:
                riesgo = "Alto"
            else:
                riesgo = "Muy grave"

            resultado_texto.value = f"Índice Paquete-Año: {paquete_año:.2f}"
            riesgo_texto.value = f"Riesgo: {riesgo}"
            formula_text.value = (
                f"Fórmula: ({cigarrillos} x {años}) ÷ 20 = {paquete_año:.2f}"
            )
    ...
    años_field.on_change = calcular_paquete_anio
        ...
        controls=[
            ft.ExpansionPanel(
                ...
                header=ft.ListTile(
                    ...
                    subtitle=ft.Text("Indice de riesgo para pacientes fumadores", size=SUBTITLE_SIZE, color=TEXT_COLOR)
                ),
                content=ft.Container(
                    content=ft.Column(
                        controls=[
