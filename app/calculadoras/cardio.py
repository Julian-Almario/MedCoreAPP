...
def pesi_score_tep():
    ...
    edad_field = ft.TextField(
        ...
        width=200,
        text_align=ft.TextAlign.CENTER,
        on_change=lambda e: calcular_pesi(),
    )
    ...
    def calcular_pesi(e=None):
        ...
        resultado_texto.value = f"Puntaje PESI: {puntaje}"

        if puntaje <= 65:
            riesgo = "Clase I - Riesgo muy bajo con mortalidad 1.6%"
            riesgo_texto.color = "green"
        elif 66 <= puntaje <= 85:
            riesgo = "Clase II - Riesgo bajo con mortalidad 3.5%"
            riesgo_texto.color = "green"
        elif 86 <= puntaje <= 105:
            riesgo = "Clase III - Riesgo intermedio con mortalidad 7.1"
            riesgo_texto.color = "orange"
        elif 106 <= puntaje <= 125:
            riesgo = "Clase IV - Riesgo alto con mortalidad 11.4%"
            riesgo_texto.color = "red"
        else:
            riesgo = "Clase V - Riesgo muy alto con mortalidad 24.5%"
            riesgo_texto.color = "grey"
    ...
    def construir_tabla(criterios, checkboxes):
        filas = []
        for texto, valor in criterios:
            chk = ft.Checkbox(value=False, on_change=calcular_pesi)
            checkboxes.append((chk, valor))
            fila = ft.Row(
                controls=[
                    ft.Container(ft.Text(texto, color=TEXT_COLOR), expand=True),
                    ft.Container(ft.Text(f"+{valor}", color=TEXT_COLOR), width=40),
                    ft.Container(chk)
                ],
                alignment=ft.MainAxisAlignment.SPACE_BETWEEN
            )
    ...
    def on_expand_change(e):
        ...
        if not panel.expanded:
            ...
            for cb, _ in checkboxes:
                cb.value = False
                cb.update()
            resultado_texto.value = "Puntaje PESI: -"
            riesgo_texto.value = ""
            resultado_texto.update()
            riesgo_texto.update()
    ...
    return ft.ExpansionPanelList(
        ...
        controls=[
            ft.ExpansionPanel(
                ref=panel_ref,
                header=ft.ListTile(
                    title=ft.Text("PESI Score Tromboembolismo", color=TEXT_COLOR),
                    subtitle=ft.Text(
                        "Índice de severidad y riesgo de mortalidad para tromboembolismo en 30 dias",
                        size=SUBTITLE_SIZE,
                        color=TEXT_COLOR
                    )
                ),
